...
JOB_DESCRIPTIONS_DIR = os.getenv(
...
THROTTLE_DELAY = float(
    os.getenv("THROTTLE_DELAY", "1.5")
)  # Delay in seconds to prevent hitting LLM rate limits
...
def get_llm_model(
    provider: str, model_name: str, api_key: str, api_url: Optional[str] = None
):
    if provider == "Groq":
