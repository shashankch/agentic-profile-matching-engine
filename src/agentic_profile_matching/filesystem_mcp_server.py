...
def watch_directory(directory: str) -> Dict:
    ...
    if not path.exists() or not path.is_dir():
        return {
            "success": False,
            "error": f"Path is not an existing directory: {directory}",
        }
    ...
    with _watcher_lock:
        if abs_dir in _active_watchers and _active_watchers[abs_dir]["active"]:
            return {
                "success": True,
                "message": f"Already actively watching directory: {abs_dir}",
            }
    ...
    return {
        "success": True,
        "message": f"Successfully started watching directory: {abs_dir}",
    }
