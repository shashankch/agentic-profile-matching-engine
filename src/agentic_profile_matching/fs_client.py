...
def read_file(filepath: str) -> Dict:
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing read_file to MCP filesystem server for path: {filepath}"
        )
        return mcp_client.call_tool("filesystem", "read_file", {"filepath": filepath})

    logger.info(
        f"[fs_client] Routing read_file directly (local mode) for path: {filepath}"
    )
...
def list_files(directory: str, extension: Optional[str] = None) -> List[Dict]:
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing list_files to MCP filesystem server for dir: {directory}"
        )
        return mcp_client.call_tool(
            "filesystem", "list_files", {"directory": directory, "extension": extension}
        )

    logger.info(
        f"[fs_client] Routing list_files directly (local mode) for dir: {directory}"
    )
...
def write_file(filepath: str, content: str) -> Dict:
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing write_file to MCP filesystem server for path: {filepath}"
        )
        return mcp_client.call_tool(
            "filesystem", "write_file", {"filepath": filepath, "content": content}
        )

    logger.info(
        f"[fs_client] Routing write_file directly (local mode) for path: {filepath}"
    )
...
def search_in_file(
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing search_in_file to MCP filesystem server for: {filepath}"
        )
        return mcp_client.call_tool(
            "filesystem",
            "search_in_file",
    ...
    logger.info(
        f"[fs_client] Routing search_in_file directly (local mode) for: {filepath}"
    )
...
def watch_directory(directory: str) -> Dict:
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing watch_directory to MCP filesystem server: {directory}"
        )
        return mcp_client.call_tool(
            "filesystem", "watch_directory", {"directory": directory}
        )

    logger.info(
        f"[fs_client] Starting local watch_directory background monitor (local mode) for: {directory}"
    )
...
def batch_process(filepaths: List[str]) -> List[Dict]:
    ...
    if getattr(config, "USE_MCP", False):
        logger.info(
            f"[fs_client] Routing batch_process to MCP filesystem server for {len(filepaths)} files"
        )
        res = mcp_client.call_tool(
            "filesystem", "batch_process", {"filepaths": filepaths}
        )
    ...
    logger.info(
        f"[fs_client] Running local batch_process concurrently (local mode) for {len(filepaths)} files"
    )
    from agentic_profile_matching.filesystem_mcp_server import (
